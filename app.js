...
function evaluateImmunizations(person, selectedKeys, today, projDate, thresholds) {
  ...
  selectedKeys.forEach(key => {
    ...
    if(res.includeInReport) {
      includeGrouped = true;
      if(res.status !== STATUS.OK) { count++; dueNames.push(imm.label); }
    }
  });
  return {worstStatus, count, dueNames, perVaccine, includeGrouped};
}
...
function evaluatePerson(person, settings, today, projDate) {
  ...
  const immunizationsResult = {
    grouped:{status:immRes.worstStatus, count:immRes.count, dueNames:immRes.dueNames, includeInReport:immRes.includeGrouped},
  ...
  const allResults = [
    ...
    tstResult, tstQuestResult, wellWomanResult,
  ];
...
function getCellHTML(result, col, settings){
  ...
  if(type === 'imm-grouped'){
    ...
    return`<td class="col-item ${statusToCss(grp.status)}"><div class="imm-grouped" data-tooltip="${escHtml(grp.dueNames.join(', '))}">${grp.count} due</div></td>`;
  }
...
// Replaces the old CSS :hover approach which was clipped by overflow:hidden.
function wireImmTooltips() {
  const tip = dom.floatingTip;
  document.querySelectorAll('.imm-grouped').forEach(cell => {
    const names = (cell.dataset.tooltip || '').split(', ').filter(Boolean);
    if(!names.length) return;
    cell.addEventListener('mouseenter', e => {
      tip.innerHTML = `<strong>Due immunizations</strong><p>${names.map(n=>escHtml(n)).join('<br/>')}</p>`;
      tip.classList.add('visible');
      placeImmTip(e);
    });
