...
function exportWrapperAsPdf(wrapperId) {
  ...
  for (let j = insertAt.length - 1; j >= 0; j--) {
    // (b) Column-name clone — classed so print CSS can style it
    if (colNamesEl) {
      const colClone = colNamesEl.cloneNode(true);
      colClone.className = 'print-col-names-repeat';
      if (tbody) tbody.insertBefore(colClone, rows[insertAt[j]]);
    }

    // (a) Spacer — page break + blank zone for fixed header
    ...
    if (tbody) tbody.insertBefore(spacer, rows[insertAt[j]]);
  }
