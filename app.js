...
// Copies the targeted wrapper into #printOutput, then injects the header
// and legend as a prepended <tr> inside the existing .hitlist-table <thead>.
// The browser then natively repeats the full <thead> (header + legend +
// column names) on every printed page.
function exportWrapperAsPdf(wrapperId) {
  ...
  const wrapper = dom.printOutput.querySelector('.hitlist-wrapper');
  ...
  if(header && legend && thead) {
    // Count columns from the existing header row
    const colCount = thead.querySelectorAll('th').length;

    // Build a single spanning cell containing header + legend
    const hCell = document.createElement('td');
    hCell.setAttribute('colspan', colCount);
    hCell.className = 'print-header-cell';
    hCell.appendChild(header);   // moves .hitlist-header into thead
    hCell.appendChild(legend);   // moves .hitlist-legend into thead

    // Prepend as the first row of the existing thead
    const hRow = document.createElement('tr');
    hRow.appendChild(hCell);
    thead.insertBefore(hRow, thead.firstChild);
  }

  // Force overflow:visible inline so Chrome does not treat the table wrapper
  // as a scroll container — scroll containers suppress <thead> repetition in print.
  const tableWrap = wrapper.querySelector('.hitlist-table-wrapper');
