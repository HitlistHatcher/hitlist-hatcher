...
//   • Pages 2+: a .print-page-spacer <tr> is injected before every P-th data
//              row. Each spacer has break-before:page (forces a page break) and
//              height:headerH (reserves the zone the fixed header occupies),
//              so the first visible data row on each new page starts exactly
//              below the header.
//
...
function exportWrapperAsPdf(wrapperId) {
  ...
  const colCount   = dom.printOutput.querySelectorAll('.hitlist-table thead th').length || 1;
  ...
  // Desired order at each break point:  ...data | spacer | colClone | rows[P]...
  // Strategy: insert colClone before rows[P], then insert spacer before colClone.
  // Keeping a direct reference to colClone avoids any querySelector fragility.
  for (let j = insertAt.length - 1; j >= 0; j--) {
    const target = rows[insertAt[j]];

    // Step 1: insert colClone before target → ...data, colClone, target...
    let colClone = null;
    if (colNamesEl && tbody) {
      colClone = colNamesEl.cloneNode(true);
      colClone.className = 'print-col-names-repeat';
      tbody.insertBefore(colClone, target);
    }

    // Step 2: insert spacer before colClone → ...data, spacer, colClone, target...
    // (spacer carries break-before:page so the page break fires before colClone)
    ...
    if (tbody) tbody.insertBefore(spacer, colClone || target);
  }
  ...
  styleEl.textContent =
    `@media print {
       ...
       /* Spacer rows: force page break and reserve header zone on each new page */
       .print-page-spacer { display: table-row !important; break-before: page; page-break-before: always; }
       .print-page-spacer td { height: ${headerH}px !important; padding: 0 !important; border: none !important; background: transparent !important; }
       /* Column-name clone rows: style to match the real <thead> row */
