...
function applySettings(s) {
  ...
  // Rebuild column editors with saved content
  const count = parseInt(colCount, 10) || 1;
  dom.infoColContainer.innerHTML = '';
  for(let i = 0; i < count; i++) {
    const prev = savedCols[i] || { text: '', align: 'center' };
    const wrapper = document.createElement('div');
    wrapper.className = 'info-col-editor';
    wrapper.innerHTML = `
      <div class="info-col-header">
      ...
      <textarea class="settings-textarea info-col-textarea" rows="4"
        placeholder="Clinic hours, phone numbers...">${escHtml(prev.text)}</textarea>`;
    wrapper.querySelector('textarea').addEventListener('input',   liveUpdateInfoBar);
    wrapper.querySelector('select').addEventListener('change',    liveUpdateInfoBar);
    dom.infoColContainer.appendChild(wrapper);
  }

  // TST sub visibility
  const eitherTst = items.tst || items.tstQuest;
  dom.tstSub.classList.toggle('hidden', !eitherTst);
