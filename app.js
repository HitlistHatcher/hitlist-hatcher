...
const IMMUNIZATION_DEFAULTS = {
  adenovirus:false, anthrax:false, cholera:false, hepa:false, hepb:false, hpv:false,
  influenza:true,   jev:false,     mgc:false,     mmr:false,  pneumo:false, polio:false,
  rabies:false,     rabiesTiter:false, smallpox:false, tdap:true, twinrix:false,
  typhoid:false,    varicella:false,   yellowFever:false,
};
...
const WT_STEPS = [
  ...
  },
  {
    ...
    target:'immSection', side:'right', scrollTo:'reportItemsCard',
  },
  {
    ...
    target:'deploymentGroup', side:'right', scrollTo:'reportItemsCard',
  },
  {
  ...
  },
  {
    ...
    target:'unitNameRow', side:'right', scrollTo:'unitCard',
  },
  {
...
function wtShowStep(idx) {
  ...
  // We scroll .settings-scroll directly (offsetTop-based) rather than using
  // scrollIntoView(), which is unreliable inside nested scroll containers and
  // with block:'nearest' won't scroll when the card top is already visible but
  // the actual target sub-group is still below the fold.
  ...
  if(scrollTarget && settingsScroll && settingsScroll.contains(scrollTarget)) {
    const SCROLL_PAD = 12; // px gap above the target element
    settingsScroll.scrollTop = scrollTarget.offsetTop - SCROLL_PAD;
  }
...
function wtPositionSpotlight(step) {
  ...
  dom.wtSpotlight.style.top    = (r.top    - PAD) + 'px';
  dom.wtSpotlight.style.left   = (r.left   - PAD) + 'px';
  dom.wtSpotlight.style.width  = (r.width  + PAD*2) + 'px';
  dom.wtSpotlight.style.height = (r.height + PAD*2) + 'px';
}
...
function initInfoColumns(){
  ...
  // Preserve existing text/align values before rebuilding
  const existing=[];
  ...
  for(let i=0;i<count;i++){
    // Default alignment is center for new columns
    ...
    ta.addEventListener('input',   liveUpdateInfoBar);
    sel.addEventListener('change', liveUpdateInfoBar);
...
// A4 landscape: 11.69in wide, 0.35in left + 0.35in right margins
// At CSS 96dpi: (11.69 - 0.70) * 96 ≈ 1055px — used for accurate header measurement
const PRINT_CONTENT_WIDTH = 1055;

// Core export helper — used by all three export buttons.
// Copies the targeted wrapper into #printOutput, measures at print width,
// injects the padding style, then triggers window.print().
function exportWrapperAsPdf(wrapperId) {
  ...
  // Reveal off-screen at print width so offsetHeight is accurate
  dom.printOutput.style.display    = 'block';
  dom.printOutput.style.visibility = 'hidden';
  dom.printOutput.style.position   = 'absolute';
  dom.printOutput.style.top        = '-99999px';
  dom.printOutput.style.width      = PRINT_CONTENT_WIDTH + 'px';

  requestAnimationFrame(() => {
    const header  = dom.printOutput.querySelector('.hitlist-header');
    const legend  = dom.printOutput.querySelector('.hitlist-legend');
    const headerH = header ? header.offsetHeight : 0;
    const legendH = legend ? legend.offsetHeight : 0;
    const totalPad = headerH + legendH + 4;

    // Reset inline styles — @media print will show the element during printing
    ...
    styleEl.textContent =
      `@media print {
         .print-only .hitlist-legend        { top: ${headerH}px; }
         .print-only .hitlist-table-wrapper { padding-top: ${totalPad}px; }
       }`;
