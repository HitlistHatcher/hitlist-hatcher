...
const WT_STEPS = [
  ...
  },
  {
    ...
    target:'reportItemsCard', side:'right', scrollTo:'reportItemsCard',
  },
  {
  ...
  },
  {
    ...
    target:'settingsPanel', side:'right', scrollTo:null,
  },
  {
  ...
  },
  {
    ...
    target:'reportOutput', side:'left', scrollTo:null,
  },
  {
  ...
  {
    title: 'Export to PDF',
    desc:  'Click the <strong>Export PDF</strong> button below the report. In the print dialog, set the destination to <strong>"Save as PDF."</strong> The full header repeats on every page so each printed page is self-contained.',
...
function renderReport(results, stats, settings, today, projDate){
  ...
  if(settings.offEnlFilter==='separate'){
    ...
    let html='';
    if(officers.length) html+=buildHitListHTML(officers,stats,settings,today,projDate,activeOff,'Officer Hit List');
    if(enlisted.length) html+=`<div style="margin-top:32px;">${buildHitListHTML(enlisted,stats,settings,today,projDate,activeEnl,'Enlisted Hit List')}</div>`;
    dom.previewPlaceholder.classList.add('hidden');
  ...
  dom.previewPlaceholder.classList.add('hidden');
  dom.reportOutput.innerHTML=buildHitListHTML(results,stats,settings,today,projDate,activeColDefs,null);
  wireImmTooltips();
  wireTstTooltip();
  dom.exportBar.hidden=false; state.reportGenerated=true;
}

function buildHitListHTML(results, stats, settings, today, projDate, colDefs, subtitleOverride){
...
function wireExportPdfHandler(){
  dom.exportPdfBtn.addEventListener('click',()=>{
    // Copy report HTML into print-only container
    dom.printOutput.innerHTML = dom.reportOutput.innerHTML;

    // Measure rendered header + legend height so table doesn't hide behind fixed header.
    // Legend top is also set explicitly so it lands directly below the header.
    requestAnimationFrame(() => {
