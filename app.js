...
//
// Strategy: position:fixed repeats the header on every printed page.
// The problem with fixed headers is they overlap page content on pages 2+.
// The solution is to set @page { margin-top } dynamically to the exact
// height of the fixed header — creating a reserved zone at the top of
// every page that the header occupies without covering any rows.
//
// Measurement is done off-screen at print width with print font-size
// overrides applied inline so the measured height matches print reality.
const PRINT_PAGE_MARGIN   = 0.35; // inches — matches @page margin in CSS
const PRINT_PAGE_MARGIN_B = 0.45; // bottom margin (slightly larger)
const PRINT_WIDTH_PX      = 1055; // A4 landscape at 96dpi minus side margins
...
function exportWrapperAsPdf(wrapperId) {
  ...
  // ── Measure header height at print dimensions ───────────────
  // Reveal off-screen at print width so offsetHeight is accurate.
  dom.printOutput.style.cssText =
    `display:block;visibility:hidden;position:absolute;top:-99999px;width:${PRINT_WIDTH_PX}px;`;

  // Apply print font-size overrides inline before measuring so the
  // measured height matches what the browser will render in @media print.
  ...
  // Force layout then read heights
  const headerH = pHeader ? pHeader.offsetHeight : 0;
  const legendH = pLegend ? pLegend.offsetHeight : 0;
  const totalH  = headerH + legendH;

  // Reset inline styles — @media print takes over during printing
  dom.printOutput.style.cssText = '';
  if(pName)  pName.style.fontSize  = '';
  if(pTitle) pTitle.style.fontSize = '';

  // ── Set legend position and @page margin-top ────────────────
  // Legend sits directly below header in fixed positioning.
  // @page margin-top = header+legend height in inches, so every page's
  // content area starts below the fixed header with no overlap.
  const marginTopIn = (totalH / 96) + PRINT_PAGE_MARGIN;
  ...
  styleEl.textContent =
    `@media print {
       @page { margin: ${marginTopIn.toFixed(4)}in ${PRINT_PAGE_MARGIN}in ${PRINT_PAGE_MARGIN_B}in ${PRINT_PAGE_MARGIN}in; }
