...
// Extracts .hitlist-table from the source wrapper and places it as a
// direct child of #printOutput with no intervening divs — this is
// required for Chrome's print engine to repeat <thead> on every page.
// The header and legend are injected as the first <thead> row so they
// repeat natively alongside the column name row.
function exportWrapperAsPdf(wrapperId) {
  ...
  // Clone into a temporary container so we can extract elements cleanly
  const temp = document.createElement('div');
  temp.innerHTML = source.outerHTML;
  const wrapper = temp.querySelector('.hitlist-wrapper');

  const header = wrapper.querySelector('.hitlist-header');
  const legend = wrapper.querySelector('.hitlist-legend');
  const table  = wrapper.querySelector('.hitlist-table');
  const thead  = table ? table.querySelector('thead') : null;

  if(!table || !thead) return;

  // Clear any stale printHeaderPad styles from prior sessions
  const staleStyle = document.getElementById('printHeaderPad');
  if(staleStyle) staleStyle.textContent = '';

  if(header && legend) {
    // Count columns from the existing column-name row
    const colCount = thead.querySelectorAll('th').length;

    // Wrap header + legend in a single full-width spanning cell
    const hCell = document.createElement('td');
    hCell.setAttribute('colspan', colCount);
    hCell.className = 'print-header-cell';
    hCell.appendChild(header);  // .hitlist-header moves into thead
    hCell.appendChild(legend);  // .hitlist-legend moves into thead

    // Prepend as the very first row of thead
    const hRow = document.createElement('tr');
    hRow.appendChild(hCell);
    thead.insertBefore(hRow, thead.firstChild);
  }

  // Place the table as a direct child of #printOutput — no wrapper divs.
  // Chrome suppresses <thead> repetition when the table is nested inside
  // any overflow or block-formatting-context element.
  dom.printOutput.innerHTML = '';
  dom.printOutput.appendChild(table);
