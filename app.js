...
//
// Strategy: inject .hitlist-header and .hitlist-legend as the first row
// of the existing .hitlist-table <thead>, then place the table as a
// direct child of #printOutput with no wrapper divs. The browser then
// repeats the full <thead> (header + legend + column names) on every page.
//
// Critical: @media print sets .hitlist-header and .hitlist-legend to
// position:static so no fixed elements exist inside <thead> — Chrome
// suppresses <thead> repetition if any fixed element is present inside it.
function exportWrapperAsPdf(wrapperId) {
  ...
  // Clone into a temp container so we can extract elements without
  // mutating the live report DOM
  const temp = document.createElement('div');
  temp.innerHTML = source.outerHTML;

  const header = temp.querySelector('.hitlist-header');
  const legend = temp.querySelector('.hitlist-legend');
  const table  = temp.querySelector('.hitlist-table');
  ...
  if(header && legend) {
    ...
    hCell.appendChild(header);
    hCell.appendChild(legend);
  ...
  // Place the table as the sole direct child of #printOutput.
  // No wrapper divs — overflow containers suppress <thead> repetition.
