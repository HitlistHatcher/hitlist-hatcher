...
// Copies the targeted wrapper into #printOutput, transforms it so that
// .hitlist-header and .hitlist-legend live inside a <thead> — the browser
// then repeats them natively on every printed page with no JS measurement
// or padding injection required.
function exportWrapperAsPdf(wrapperId) {
  ...
  // Clear any stale printHeaderPad styles left over from a prior session
  const staleStyle = document.getElementById('printHeaderPad');
  if(staleStyle) staleStyle.textContent = '';

  // Move header + legend into a <thead> so they repeat on every page
  ...
  // Outer single-column table — thead repeats, tbody holds data
  const outerTable = document.createElement('table');
  outerTable.className = 'print-outer-table';

  const thead  = document.createElement('thead');
  const hRow   = document.createElement('tr');
  const hCell  = document.createElement('td');
  hCell.appendChild(header);    // moves .hitlist-header into thead cell
  hCell.appendChild(legend);    // moves .hitlist-legend into thead cell
  ...
  bCell.appendChild(tableWrap); // moves .hitlist-table-wrapper into tbody cell
